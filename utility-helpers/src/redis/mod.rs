...
use crate::redis::redis_async_pool::RedisConnectionManager;
...
pub async fn new(url: &str, cache_expiry: u64) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    // ✅ modern async connection (NOT deprecated)
    let mut con = client.get_multiplexed_async_connection().await?;

    // ✅ safe ping test
    let pong: String = redis::cmd("PING").query_async(&mut con).await?;

    log_info!("Redis PING response: {}", pong);

    let manager = RedisConnectionManager::new(client, false, None);
   ...
   // --- 3. Drop the connection BEFORE running the callback ---
    drop(conn);
  ...
  // --- 5. Get connection again only when needed ---
    let mut conn = self.pool.get().await?;
