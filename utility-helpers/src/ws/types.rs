...
pub enum ChannelType {
    ...
    OrderBookPoster,
    BitcoinPrice(String),  // ✅ Add this - symbol like "BTCUSDT"
}
...
impl ChannelType {
    pub fn from_str(s: &str) -> Option<ChannelType> {
        ...
        } else if s.starts_with("bitcoin_price:") {  // ✅ Add this
            let symbol = s.strip_prefix("bitcoin_price:").unwrap_or("").to_string();
            if !symbol.is_empty() {
                return Some(ChannelType::BitcoinPrice(symbol));
            }
            return None;
   ...
   pub fn to_str(&self) -> String {
        match self {
            ...
            ChannelType::BitcoinPrice(symbol) => format!("bitcoin_price:{symbol}"),  // ✅ Add this
