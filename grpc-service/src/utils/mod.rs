...
pub mod clickhouse_queries {
    ...
    pub const MARKET_VOLUME_BASE_QUERY: &str = r#"
            SELECT
                ...
                -- YES - BUY
                toFloat64(SUM(if(outcome = 'yes' AND side = 'buy', shares, 0))) AS yes_buy_qty,
                toFloat64(SUM(if(outcome = 'yes' AND side = 'buy', quantity, 0))) AS yes_buy_usd,

                -- YES - SELL
                toFloat64(SUM(if(outcome = 'yes' AND side = 'sell', shares, 0))) AS yes_sell_qty,
                toFloat64(SUM(if(outcome = 'yes' AND side = 'sell', quantity, 0))) AS yes_sell_usd,

                -- NO - BUY
                toFloat64(SUM(if(outcome = 'no' AND side = 'buy', shares, 0))) AS no_buy_qty,
                toFloat64(SUM(if(outcome = 'no' AND side = 'buy', quantity, 0))) AS no_buy_usd,

                -- NO - SELL
                toFloat64(SUM(if(outcome = 'no' AND side = 'sell', shares, 0))) AS no_sell_qty,
                toFloat64(SUM(if(outcome = 'no' AND side = 'sell', quantity, 0))) AS no_sell_usd
