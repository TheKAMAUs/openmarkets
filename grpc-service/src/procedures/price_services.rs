...
async fn get_price_data_within_interval(
    ...
    let market_id = Uuid::parse_str(&market_id)
        .map_err(|_| Status::invalid_argument("Invalid market_id"))?;

    let time_range = Timeframe::try_from(timeframe).map_err(|_| {
        Status::invalid_argument("Invalid timeframe provided")
    })?;

    let now = chrono::Utc::now().naive_utc();

let start_time = match time_range.get_start_time() {
    Some(start) => start,
    None => {
        // For "ALL", go back 3 years (or since market creation)
        Utc::now() - chrono::Duration::days(3 * 365)  // 3 years
    }
};

let naive_start_time = start_time.naive_utc();

    // -----------------------------
    // POSTGRES REPLACEMENT QUERY
    // -----------------------------
    let resp = MarketStateHistory::get_market_state_history_by_interval(
        &self.state.db_pool,
        market_id,
        naive_start_time,
        now,
    ...
    .await
    .map_err(|e| {
        Status::internal(format!("DB query failed: {}", e))
    })?;
    ...
    let response = GetMarketPriceDataWithinIntervalResponse {
        ...
        price_data: resp
            .into_iter()
            .map(|row| PriceData {
            yes_price: row.price_yes.to_string().parse::<f64>().unwrap_or(0.0),
no_price: row.price_no.to_string().parse::<f64>().unwrap_or(0.0),
...
mod test {
    ...
    async fn test_clickhouse_data() {
        ...
        let another_query = client
            ...
            .fetch_all::<GetMarketPrices>()
            .await;
        log_info!("Another query response: {another_query:#?}");
