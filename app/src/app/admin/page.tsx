...
//     const handleChildChange = (field: keyof CreateChildMarketRequest, value: string | number) => {
...
//     // Update your openChildDialog function
//     const openChildDialog = (index?: number) => {
//         if (index !== undefined) {
//             const child = form.child_markets?.[index];
//             if (child) {
...
//     const saveChildMarket = (data: ChildMarketData) => {
//         // Validate the data from the modal
//         if (!data.name || !data.question || !data.liquidity || !data.market_expiry) {
//             toaster.error({
//                 title: "Validation Error",
//                 description: "Name, question, liquidity, and expiry are required for child markets",
...
//             if (editingChildIndex !== null) {
//                 updatedChildren[editingChildIndex] = data;
//             } else {
...
//         // Generate a guaranteed unique slug with timestamp + random string
//         const generateUniqueSlug = (name: string, suffix: string = ''): string => {
//             const baseSlug = name
//                 .toLowerCase()
//                 .replace(/[^a-z0-9]+/g, '-')
...
//             // Add timestamp and random string for guaranteed uniqueness
//             const uniqueId = `${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
//             return `${baseSlug}-${uniqueId}${suffix}`;
//         };

//         // Generate parent slug with its own unique ID
//         const parentSlug = form.slug || generateUniqueSlug(form.name);

//         // Prepare the payload with ABSOLUTELY UNIQUE slugs for ALL markets
//         const payload: CreateMarketRequest = {
//             name: form.name,
//             description: form.description,
//             logo: form.logo || undefined,
//             liquidity_b: form.liquidity_b,
//             market_expiry: form.market_expiry,
//             slug: parentSlug,
//             is_event: form.is_event || false,
//             category: form.category || undefined,
//             resolution_criteria: form.resolution_criteria || undefined,
//             child_markets: form.child_markets?.map((child, index) => {
//                 // Generate a COMPLETELY DIFFERENT unique slug for each child
//                 const childSlug = child.slug || generateUniqueSlug(child.name || 'child', `-${index}`);
//                 console.log(`📝 Child ${index} slug:`, childSlug); // Should be a stri
//                 return {
//                     name: child.name,
//                     question: child.question,
...
//         // Log ALL slugs to verify uniqueness
//         // Log each slug to verify they're strings
//         console.log("🚀 PARENT SLUG:", payload.slug, typeof payload.slug);
//         payload.child_markets?.forEach((c, i) => {
//             console.log(`🚀 CHILD ${i} SLUG:`, c.slug, typeof c.slug);
//         });

//         // Check for duplicates in the payload itself
//         const allSlugs = [
//             payload.slug,
//             ...(payload.child_markets?.map(c => c.slug) || [])
//         ].filter(Boolean);
...
//                     {/* Event Toggle - Fixed for Chakra v3 */}
//                     <Box p={4} bg="gray.50" borderRadius="md">
//                         <Field.Root>
//                             <HStack justify="space-between">
//                                 <Field.Label>Is this an Event? (Will contain child markets)</Field.Label>
...
//                                     colorScheme="blue"
//                                     onClick={() => {
//                                         console.log("➕ Add Child Market button clicked");
//                                         resetChildForm();
//                                         setChildModalOpen(true); // Use the new state
//                                     }}
//                                 >
//                                     <Plus size={16} style={{ marginRight: '4px' }} />
//                                     Add Child Market
...
//             {/* Child Market Dialog */}
//             {/* // Add the modal to your JSX (before closing Box tag) */}
//             <ChildMarketModal
//                 isOpen={childModalOpen}
//                 isEditing={editingChildIndex !== null}
//                 initialData={editingChild}
