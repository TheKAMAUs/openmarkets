...
// Update the function to accept marketPrices
function estimateValue(shares: string, outcome: string, marketPrice: any) {
  const shareCount = parseFloat(shares);
  const price = outcome.toLowerCase() === "yes"
    ? marketPrice?.latestYesPrice || 0.5
    : marketPrice?.latestNoPrice || 0.5;
  return shareCount * price;
}

function MarketPriceFetcher({
  marketId,
  onUpdate,
}: {
  marketId: string;
  onUpdate: (id: string, price: MarketPriceData) => void;
}) {
  const price = useMarketPrice(marketId);

  useEffect(() => {
    onUpdate(marketId, price);
  }, [marketId, price, onUpdate]);
...
export default function HoldingsPage() {
  ...
  const handlePriceUpdate = useCallback(
    (id: string, price: MarketPriceData) => {
  ...
  const uniqueMarketIds = [...new Set(holdingsData.map(h => h.market_id))];
  ...
  const estimatedValue = holdingsData.reduce(
    (sum, holding) => {
      const marketPrice = marketPrices[holding.market_id];
      return sum + estimateValue(holding.shares, holding.outcome, marketPrice);
    },
    0,
  );
  ...
  return (
    <Container maxW="7xl" py={8}>
      ...
      {/* Mount price fetchers */}
      {/* Fetch prices (non-visual) */}
          ...
          <CardBody p={0}>
            ...
            >
              {holdingsData.map((holding, index) => {
                ...
                return (
                  <GridItem key={`${holding.market_id}-${holding.outcome}-${index}`}
                    p={6}
