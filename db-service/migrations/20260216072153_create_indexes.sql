...
ON polymarket.orders (market_id, created_at DESC)
INCLUDE (id, market_id, user_id, price, quantity, side, order_type);
...
CREATE INDEX  IF NOT EXISTS idx_markets_name_updated_at 
ON polymarket.markets(name, updated_at DESC);
...
ON polymarket.users (last_login DESC)
...
CREATE INDEX  IF NOT EXISTS idx_user_trades_user_market
  ON polymarket.user_trades (user_id, market_id)
  INCLUDE (created_at);
...
CREATE INDEX  IF NOT EXISTS idx_user_trades_market_timestamp 
ON polymarket.user_trades (market_id, timestamp DESC) 
INCLUDE (user_id, trade_type, outcome, price, quantity);
