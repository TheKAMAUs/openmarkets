...
use crate::websocket::price_scheduler::start_btc_ws_scheduler;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  tracing_subscriber::fmt()
    ...
    // ✅ Start BTC price scheduler - just pass Arc<AppState>
    log_info!("🚀 Starting BTC price scheduler...");
    let price_app_state = Arc::clone(&app_state);
    start_btc_ws_scheduler(price_app_state).await;
    log_info!("✅ BTC price scheduler started");
...
async fn initialize_app() -> Result<Arc<AppState>, Box<dyn std::error::Error>> {
    ...
    let q_yes = db_order.q_yes;   // read from order record
    let q_no = db_order.q_no;     // read from order record
    let mut order: Order = db_order.into();
    global_book.process_order(&mut order, liquidity_b, q_yes, q_no);
    order_ctn += 1;
}
