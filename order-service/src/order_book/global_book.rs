...
use super::market_book::{MarketBook,   LmsrExecutionResult    };
...
pub(crate) fn process_order(
        ...
        let market_book = self.get_or_create_market(market_id, liquidity_b, q_yes, q_no);
...
pub(crate) fn add_order(&mut self,  order: &mut Order, liquidity_b: Decimal, q_yes: Decimal, q_no: Decimal) {
    let market_id = order.market_id;
    let market_book = self.get_or_create_market(market_id, liquidity_b, q_yes, q_no);
    market_book.process_order(order);
}
...
pub(crate) fn create_market_order(
    ...
    // Use get_or_create_market with the provided q_yes and q_no
     let market_book = self.get_or_create_market(market_id, liquidity_b, q_yes, q_no);
   ...
   fn get_or_create_market(
    ...
    self.markets
        .entry(market_id)
        .and_modify(|book| {
            if liquidity_b > Decimal::ZERO && book.liquidity_b != liquidity_b {
                log_info!(
                    "Market {} — updating liquidity_b: {} → {}, q_yes: {} → {}, q_no: {} → {}",
                    market_id, book.liquidity_b, liquidity_b, book.q_yes, q_yes, book.q_no, q_no
                );
                book.liquidity_b = liquidity_b;
                book.q_yes = q_yes;
                book.q_no  = q_no;
                book.current_yes_price = book.calculate_price(Outcome::YES);
                book.current_no_price  = book.calculate_price(Outcome::NO);
            }
        })
        .or_insert(MarketBook::new(liquidity_b, q_yes, q_no))
}
}
