...
pub(super) fn match_order(&mut self, order: &mut Order) -> Vec<OrderBookMatchedOutput> {
    ...
    // log_info!(
    //     "🎯 [MATCH_ORDER] Starting match for order: id={}, user={}, side={:?}, type={:?}, quantity={}, price={}",
    //     order.id, order.user_id, order.side, order.order_type, order.quantity, order.price
    // );

    if order.status != OrderStatus::OPEN {
        // log_info!("⏭️ [MATCH_ORDER] Order {} not OPEN, status={:?}, skipping", order.id, order.status);
        return matches; // only open orders can be matched
    }
    if order.price > Decimal::ONE && order.order_type != OrderType::MARKET {
        // log_info!(
        //     "⚠️ [MATCH_ORDER] Order {} price > 1.0: {}, skipping",
    ...
    if order.price != Decimal::ZERO && order.order_type != OrderType::MARKET {
        if order.quantity == Decimal::ZERO {
            ...
            // log_info!("🟢 [MATCH_ORDER] Order {} quantity is zero, marking as FILLED", order.id);
    ...
    let (book, is_buy) = match order.side {
        OrderSide::BUY => {
            // log_info!("🛒 [MATCH_ORDER] Order {} is BUY, matching against ASKS", order.id);
            (&mut self.asks, true) // inverse matching
        },
        OrderSide::SELL => {
            // log_info!("💰 [MATCH_ORDER] Order {} is SELL, matching against BIDS", order.id);
            (&mut self.bids, false)
        },
    };
    ...
    if is_buy {
        ...
        // log_info!("📊 [MATCH_ORDER] BUY order: sorted ASK prices ascending: {:?}", keys);
    } else {
        ...
        // log_info!("📊 [MATCH_ORDER] SELL order: sorted BID prices descending: {:?}", keys);
    }

    let mut remaining = order.quantity - order.filled_quantity;
    // log_info!("📏 [MATCH_ORDER] Order {} initial remaining quantity: {}", order.id, remaining);
    
    if remaining <= Decimal::ZERO {
        // log_info!("⏭️ [MATCH_ORDER] Order {} remaining <= 0, nothing to match", order.id);
        return matches;
    }
    ...
    for price in keys {
        ...
        if order.order_type != OrderType::MARKET {
            ...
            if (is_buy && price > order.price) || (!is_buy && price < order.price) {
                // log_info!(
                //     "🚫 [MATCH_ORDER] Order {} skipping price {} - out of bounds (limit order)",
                //     order.id, price
                // );
                continue;
            }
        ...
        if let Some(price_level) = book.get_mut(&price) {
            // log_info!(
            //     "📌 [MATCH_ORDER] Processing price level {} with {} orders",
            //     price, price_level.orders.len()
            // );
            
            for opposite_order in price_level.orders.iter_mut() {
                // log_info!(
                //     "👀 [MATCH_ORDER] Checking opposite order: id={}, user={}, total={}, filled={}",
                //     opposite_order.order_id, opposite_order.user_id, 
                ...
                {
                    // log_info!(
                    //     "🔄 [MATCH_ORDER] Skipping self-match: order={}, opposite={}, same_user={}",
                    //     order.id, opposite_order.order_id, order.user_id == opposite_order.user_id
                    // );
                    continue;
                }
                let opp_remaining =
                ...
                if opp_remaining <= Decimal::ZERO {
                    // log_info!("⏭️ [MATCH_ORDER] Opposite order {} has no remaining quantity", opposite_order.order_id);
                    continue;
                }

                let match_qty = remaining.min(opp_remaining);
                // log_info!(
                //     "🤝 [MATCH_ORDER] MATCHING: order={} with opposite={}, qty={}, price={}",
                //     order.id, opposite_order.order_id, match_qty, price
                // );

                ///// ATOMIC Operation START (trusting on parking lot's RWLock )
                opposite_order.filled_quantity += match_qty;
                // log_info!(
                //     "📝 [MATCH_ORDER] Updated opposite order {}: filled={}/{}",
                //     opposite_order.order_id, opposite_order.filled_quantity, opposite_order.total_quantity
                // );

                order.filled_quantity += match_qty;
                remaining -= match_qty;
                // log_info!(
                //     "📝 [MATCH_ORDER] Updated main order {}: filled={}/{}, remaining={}",
                //     order.id, order.filled_quantity, order.quantity, remaining
                // );
                ...
                if remaining == Decimal::ZERO {
                    // log_info!("✅ [MATCH_ORDER] Order {} fully matched, breaking", order.id);
                    break;
                }
            ...
            let before_count = price_level.orders.len();
            price_level
                .orders
                .retain(|o| o.filled_quantity < o.total_quantity);
            let after_count = price_level.orders.len();
            
            if before_count != after_count {
                // log_info!(
                //     "🗑️ [MATCH_ORDER] Removed {} filled orders from price level {}",
                //     before_count - after_count, price
                // );
            }
            ...
            // log_info!(
            //     "📊 [MATCH_ORDER] Price level {} total remaining quantity: {}",
            //     price, price_level.total_quantity
            // );

            if price_level.orders.is_empty() {
                book.remove(&price);
                // log_info!("🧹 [MATCH_ORDER] Removed empty price level {}", price);
            }
            if remaining == Decimal::ZERO {
                // log_info!("✅ [MATCH_ORDER] Order {} fully matched, breaking outer loop", order.id);
                break;
            }
        } else {
            // log_warn!("⚠️ [MATCH_ORDER] Price level {} disappeared while processing!", price);
        }
    }

    if order.filled_quantity == order.quantity {
        order.status = OrderStatus::FILLED;
        log_info!("🏁 [MATCH_ORDER] Order {} fully filled, status=FILLED", order.id);
    } else {
        // log_info!(
        //     "⏳ [MATCH_ORDER] Order {} partially filled: {}/{}, remaining={}",
        //     order.id, order.filled_quantity, order.quantity, remaining
        // );
    }

    // log_info!("🎉 [MATCH_ORDER] Completed for order {}, generated {} matches", order.id, matches.len());
    matches
}
...
fn get_available_match_quantity(&mut self, order: &mut Order, budget: Decimal) -> Decimal {
    ...
    log_info!("🛠️ Starting matching for order: {:?}", order);
    log_info!("💰 Initial budget: {}", budget);
    ...
    let book = match order.side {
        OrderSide::BUY => {
            log_info!("⬆️ Order side: BUY, matching against asks");
            &mut self.asks
        }
        OrderSide::SELL => {
            log_info!("⬇️ Order side: SELL, matching against bids");
            &mut self.bids
        }
    };
    ...
    if order.side == OrderSide::BUY {
        keys.sort_by(|a, b| a.partial_cmp(b).unwrap()); // ascending: buy from lowest
        log_info!("🔹 Sorted keys ascending (lowest price first): {:?}", keys);
    } else {
        keys.sort_by(|a, b| b.partial_cmp(a).unwrap()); // descending: sell to highest
        log_info!("🔹 Sorted keys descending (highest price first): {:?}", keys);
    }
    ...
    for price in keys {
        if remaining_budget <= Decimal::ZERO {
            log_info!("💸 Budget depleted, stopping matching");
            break;
        }
        ...
        if let Some(level) = book.get(&price) {
            ...
            for entry in &level.orders {
                if entry.user_id == order.user_id {
                    log_info!("🙅 Skipping self order: user_id {}", entry.user_id);
                    continue;
                }
                let rem_qty = entry.total_quantity - entry.filled_quantity;
                if rem_qty > Decimal::ZERO {
            ...
            let cost_to_consume_level = price * total_level_qty;
            log_info!("📊 Price: {}, total_level_qty: {}, cost_to_consume_level: {}", price, total_level_qty, cost_to_consume_level);

            if remaining_budget >= cost_to_consume_level {
                // consume full level
                log_info!("✅ Consuming full level quantity: {}", total_level_qty);
                available_quantity += total_level_qty;
                remaining_budget -= cost_to_consume_level;
                ...
                // partial consume
                let partial_qty = remaining_budget / price;
                log_info!("⚡ Partial consume: {}, remaining budget before: {}", partial_qty, remaining_budget);
                available_quantity += partial_qty;
                remaining_budget -= price * partial_qty;
                log_info!("💰 Remaining budget after partial consume: {}", remaining_budget);
                break;
            }
        }
    }
    ...
    if remaining_budget > Decimal::ZERO || budget == Decimal::ZERO {
        log_info!("❗ Order not fully matched, cancelling order");
        order.status = OrderStatus::CANCELLED;
    }

    log_info!("🏁 Finished matching. Available quantity: {}", available_quantity);
    available_quantity
}
